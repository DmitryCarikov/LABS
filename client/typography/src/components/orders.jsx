...
const OrdersPage = () => {
    ...
    const handlePriceChange = (orderId, newPrice) => {
        setPriceUpdates(prev => ({ ...prev, [orderId]: newPrice }));
    };

    const handlePriceBlur = (orderId) => {
        if (priceUpdates[orderId] !== undefined) {
            dispatch(updateOrder({ id: orderId, updatedData: { price: priceUpdates[orderId] } }));
        }
    };
                        ...
                        {filteredOrders.map((order) => (
                            <TableRow key={order._id}>
                                ...
                                <TableCell align="right">
                                    <a href={order.fileUrl} target="_blank" rel="noopener noreferrer">Скачать файл</a>
                                </TableCell>
                                <TableCell align="right">
                                    <TextField
                                        type="number"
                                        value={priceUpdates[order._id] !== undefined ? priceUpdates[order._id] : order.price}
                                        onChange={(e) => handlePriceChange(order._id, parseFloat(e.target.value))}
                                        onBlur={() => handlePriceBlur(order._id)}
                                        margin="normal"
                                    />
                                </TableCell>
                                <TableCell align="right">
